...
class APIManager(object):
    ...
    def get_doc_count(self, keywords: list):
        ...
        data = json.dumps({"token": self.textpresso_api_token, "query": {
            "keywords": " ".join(keywords), "type": "document", "corpora": ["C. elegans"]}})
    ...
    def get_doc_matching(self, keywords: list, start: int = 0, count: int = 200):
        ...
        data = json.dumps({"token": self.textpresso_api_token, "query": {
            ...
            "count": count}, "include_fulltext": True})
        data = data.encode('utf-8')
        req = urllib.request.Request(self.tpc_api_endpoint, data, headers={'Content-type': 'application/json',
                                                                           'Accept': 'application/json'})
        res = urllib.request.urlopen(req)
        logger.debug("Sending request to Textpresso Central API")
        return [(doc["identifier"], doc["accession"], doc["title"] + doc["abstract"] + doc["fulltext"]) for doc in json.loads(res.read().decode('utf-8'))]

    @staticmethod
    def get_ids_from_wb_ftp():
        ret = []
        urllib.request.urlretrieve("ftp://ftp.ebi.ac.uk/pub/databases/wormbase/releases/WS277/species/c_elegans/PRJNA13758/annotation/c_elegans.PRJNA13758.WS277.knockout_consortium_alleles.xml.gz", "ko_allelex.xml.gz")
